CMD:convidar(playerid, params[])
{
	...
	/*if(g_PlayerData[playa][pMembro] > 0)return SendClientMessage(playa,BRANCO,"Este player já possui uma Organização.");*/
	if(playa == INVALID_PLAYER_ID) return SendClientMessage(playerid, BRANCO, "Jogador não existe.");
	GetPlayerName(playa, giveplayer, sizeof(giveplayer));
	GetPlayerName(playerid, name, sizeof(name));
	new ftext[30];
	if(g_PlayerData[playerid][pLider] == 1) ftext = "LSPD";
	...
	InviteJob[playa] = g_PlayerData[playerid][pLider]; InviteOffer[playa] = playerid;//Detalhe - irá setar a variavel o ID da org/qm mandou convite
